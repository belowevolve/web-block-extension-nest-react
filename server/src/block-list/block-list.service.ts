...
import { AddBlockItemDto, BlockListQueryDto, UpdateBlockItemDto } from './dto';
...
export class BlockListService {
  ...
  async updateItem(userId: number, itemId: number, data: UpdateBlockItemDto) {
    const blockList = await this.db.blockList.findFirstOrThrow({
      where: { ownerId: userId },
    });

    return this.db.blockItem
      .update({
        where: {
      ...
      .catch(() => {
        throw new NotFoundException(`Block item ${itemId} not found`);
      });
  }
